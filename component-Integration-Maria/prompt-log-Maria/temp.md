...
---
# Entry 8 — Rebuilding the Workflow Architecture from Scratch

**Date:** May 2026

### Prompt

The original AI Quiz Generator workflows became too tightly coupled and difficult to debug due to broken node dependencies, inconsistent metadata passing, and integration failures across Airtable, n8n, and Flowise.

If I were rebuilding the system from scratch, what architecture would be cleaner and more maintainable?
...
Break the system into modular workflows instead of one tightly coupled pipeline.
...
Workflow 2 — Question Generation
- trigger when chunked text becomes ready
- send content to Flowise
- validate JSON output
- create linked question records

Workflow 3 — Quiz Delivery
- allow users to select generated quizzes through Airtable interface

Workflow 4 — Submission + Grading
...
This directly informed the final architecture used in the rebuilt capstone system.

---

# Entry 9 — Debugging Workflow Logic After Rebuild

**Date:** May 2026

### Prompt

I rebuilt the workflow architecture, and the overall system is cleaner, but some downstream automation still behaves unpredictably.

What’s the best debugging strategy for tracing workflow failures across n8n, Airtable, and Flowise integrations?
...
1. Validate each workflow independently before end-to-end testing
2. Inspect node execution data after every run
3. confirm Airtable fields contain expected values
4. verify JSON output schemas between Flowise and n8n
5. test API calls independently before chaining them
6. surface explicit error statuses instead of silent failures
7. isolate failures at the component level before reconnecting workflows

Example debugging rule:

Bad:
end-to-end testing without validating intermediate outputs

Better:
validate each component independently before full orchestration

---

### Outcome

This improved debugging efficiency during final system validation and reduced repeated integration failures.

---

## Reflection

Between Week 8 and the end of this project, my approach to prompting AI tools became significantly more structured and engineering-focused. Early on, I used prompts mainly for quick debugging fixes or one-off answers, but over time I learned to treat AI as a development collaborator by providing clearer system context, defining constraints, requesting structured outputs, and using iterative prompts to troubleshoot architecture decisions rather than isolated errors. The rebuild especially changed how I worked with AI, because instead of asking narrow “what broke?” questions, I began using prompts to audit system design, evaluate tradeoffs, redesign workflows, and validate implementation decisions more strategically.
